...
app.post("/getnexthub", function(req, res) {
  ...
  // res.json(req.body);
  if (!req.body.currentHub.hub) {
    res.send("no hub sent for search..");
    return;
  }

  let where = { name: req.body.currentHub.hub };
  let where2 = "1=1";
  if (req.body.type) {
  ...
  connection.query(
    "SELECT country, locations.id, locations.name,lat," +
      "lng,trip_id,trips.name as trip_name, trips.type as trip_type" +
      " FROM locations inner join trips on trips.id = locations.trip_id" +
      " WHERE trip_id IN ( SELECT trip_id FROM locations WHERE ?) and ?",
    [where, where2],
    function(err, result) {
      if (err) throw err;
      console.log(result);
      ...
      for (let i = 0; i < result.length; i++) {
        if (obj[result[i].trip_id]) {
          ...
          obj[result[i].trip_id] = {
            name: result[i].trip_name,
            type: result[i].trip_type,
            locations: []
          };
          obj[result[i].trip_id].locations.push(result[i]);
        }
